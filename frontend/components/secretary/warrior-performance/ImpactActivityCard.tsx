...
import { ImpactActivity } from "@/hooks/secretary/warrior-performance/useCyberWarriorPerformance";
import toast from "react-hot-toast";

interface ImpactActivityCardProps {
    activity: ImpactActivity;
    index: number;
    onUpdateLinks: (id: string, mediaLinks: any[], socialLinks: any[]) => void;
}

export default function ImpactActivityCard({
    activity,
    index,
    onUpdateLinks,
}: ImpactActivityCardProps) {
    const [mediaLinks, setMediaLinks] = useState(activity.mediaLinks);
    const [socialLinks, setSocialLinks] = useState(activity.socialMediaLinks);
    ...
    const handleAddMediaLink = () => {
        ...
        onUpdateLinks(activity.id, updatedLinks, socialLinks);
    ...
    const handleAddSocialLink = () => {
        ...
        onUpdateLinks(activity.id, mediaLinks, updatedLinks);
    ...
    return (
        <div className="bg-gradient-to-br from-purple-900/20 to-pink-900/20 border border-purple-500/30 rounded-2xl shadow-lg p-6 hover:shadow-xl hover:shadow-purple-500/10 transition-all">
            {/* Header */}
            <div className="flex items-start justify-between mb-4 pb-4 border-b border-purple-500/30">
            ...
            <div className="bg-purple-500/10 rounded-lg p-4 mb-4">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2 text-gray-400">
                        ...
                        <span className="text-sm font-medium">Participants Reached</span>
                    </div>
                    <span className="text-2xl font-bold text-white">{activity.participants}</span>
                </div>
            </div>
